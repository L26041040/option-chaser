"""v6 Task 11 Step 5: 收斂自 tests/test_webapp_v4.py（v4 spec §4/§7.7 的
render.py 渲染輸出驗證——分組表格/badge/選看/expander/`$`跳脫/glossary title）。

這些斷言本質是 render.py 的輸出驗證，與頁面身份無關；v5 app.py 的快速試算流程
現已搬到 webapp/views/quick.py（Task 9），行為與舊 app.py 一致，故 AppTest 一律
改指向 webapp/views/quick.py。`test_help_page_renders`已於 Task 8 移除（本檔案
以 Task 9 完成後的 git blob d230708 為復原基準，故本來就不含該測試）。
`test_edit_form_resubmit_triggers_new_analysis` 屬 quick.py 表單重跑行為，已
改寫遷至 tests/test_views_quick.py。

注意（fix round 1，見 task-11-report.md）：本檔為對 Task 11 執行偏差的修正——
先前實作誤刪整個 tests/test_webapp_v4.py 且僅遷移 3/8 個測試進
tests/test_views_detail.py，取代（supersede）該錯誤刪除，改依計畫 Step 5 原文
建立本檔。"""
...
def test_grouped_table_renders(monkeypatch):
    ...
    at = AppTest.from_file("webapp/views/quick.py")
    at.run()
