...
- EVENT_TYPES_V5: v5 生效 6 種（store/workspace 寫事件僅允許這些）。
...
# SCENARIO_DELETED 與 SCENARIO_REMOVED 是兩種動作，不是同一種的兩個名字：
# 前者硬刪除（檔案與 results/ 一併清掉），後者是使用者在清單上的手動移除＝
# 事件溯源軟刪除（附錄 A8.2：只標記，歷史檔案全部保留）。
