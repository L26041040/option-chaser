...
def price_axis(
    ...
    """價格軸：錨點 {現價, 目標, 最高, 最低}，範圍由劇本區間 ±10% 決定。
    ...
    **QA 修正（需求方裁示）**：上限＝劇本區間高端×1.10、下限＝低端×0.90，
    ...
    參數名 `best_price`／`worst_price` 是既有契約（API 欄位、資料庫欄位
    一路到這裡），顯示文字則已統一改成「最高／最低」——「最好／最差」
    只在看漲劇本說得通，看跌劇本的「最好」其實是價格往下跑，需求方裁示
    改用純方向性的字眼避免誤解。

    「劇本區間高端／低端」取的是那組已知值的**極值**，不是字面上的
    `max(最高價位, 現價)`：看跌劇本填進 `best_price` 的是低價、填進
    `worst_price` 的是高價，照字面取會把其中一端擠出圖外。取聯集極值在
    看漲情形下與需求方給的算式完全一致，看跌也成立。
