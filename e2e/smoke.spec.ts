 ...
 *  手造合成資料。 */
/** 單腿（Long Call）契約樣本——2026-09-09 Owner 裁示後「IV 相對位置」
 *  只服務單腿候選，Historical IV 這組 e2e 因此改用它當詳細頁的
 *  `latest_result`（原本用的 `analysis_sample.json` 是兩腿 Vertical
 *  Spread，那個 champion 現在結構上不會渲染這塊）。 */
const sampleLongCall: any = JSON.parse(
  readFileSync(
    fileURLToPath(
      new URL("../contracts/analysis_sample_long_call.json", import.meta.url)),
    "utf-8"));
...
async function routeLibrary(page: import("@playwright/test").Page, row: unknown,
  ...
  await page.route("**/api/scenarios/s1", (route) =>
    route.fulfill({ json: { ...(row as object), latest_result: detailView } }));
...
test("清單 → 詳細頁：頭條 Hero 卡、進場面板、主圖、候選策略（MVP V3／#103" +
     "資訊階層重整；SW-10／#340 起 `ScenarioContext`／`Summary` 兩張卡" +
     "整段刪除，由頭條 Hero 卡承接兩者僅有的獨有欄位）",
   async ({ page }) => {
  ...
  // 頭條 Hero 卡：現價與目標（含所需漲幅）、劇本報酬、資料來源——最後
  // 這行就是雲端對 Cboe 可達性的驗證方式。SW-10 起這是頁面上顯示這些
  // 數字的唯一位置，不必再刻意縮小範圍避免跟其他卡片撞出「找到多個」
  // 的假失敗。
  const hero = page.getByRole("region", { name: "劇本頭條" });
  await expect(hero.getByText(`$${view.meta.spot.toFixed(2)}`)).toBeVisible();
  await expect(hero.getByText("+30.0%", { exact: false })).toBeVisible();
  await expect(hero.getByText(view.meta.source, { exact: true })).toBeVisible();
  ...
  await expect(hero).toContainText(`${(top.baseline_return * 100).toFixed(1)}%`);

  // 進場面板：逐腿最差成交價＋淨成本。SW-10 起「買腿 Ask」／「賣腿
  // Bid」這兩句 `Summary` 專屬標籤已經不存在——沿用既有 `EntryPanel`
  // 逐腿列格式（履約價本身當列標籤），跟到期日排名表每一列同一口徑。
  const entryPanel = page.locator("section")
    .filter({ hasText: "進場 · 以最差成交價計算" }).first();
  await expect(entryPanel).toContainText(`買 ${buy.strike}`);
  await expect(entryPanel).toContainText(`賣 ${sell.strike}`);
  await expect(entryPanel).toContainText("淨成本 / 股");
  await expect(entryPanel).toContainText(`$${top.natural_cost.toFixed(2)}`);
  // 真的整段刪除——`劇本設定`／`劇本摘要` 兩張獨立卡片，以及更早
  // 一版就已經合併掉的「基準候選」／「進場成本」兩個標題，都不存在了。
  ...
  // 主圖：只剩 Heatmap 本身——候選身分與報酬已搬到頭條 Hero 卡。
...
test("SW-10（#340，Owner 真機驗收）：手機頭條 Hero 卡呈現使用者原本建立" +
     "的劇本 context（標的／目標價／目標年月／方向／啟用的策略）——" +
     "`ScenarioContext`（劇本設定卡）／`Summary`（劇本摘要卡）已整段" +
     "刪除（OPTION-CHASER-CLOSEOUT-001 原始 AC 的「排在前面」訴求，隨" +
     "兩張卡本身一起被『同一張卡、不再重複』取代）", async ({ page }) => {
  ...
  const hero = page.getByRole("region", { name: "劇本頭條" });
  await expect(hero).toContainText("XYZ");
  await expect(hero.getByText(new RegExp(
    `目標 \\$${view.params.target_price.toFixed(2)} · ${view.params.target_month}`)))
    .toBeVisible();
  // 契約樣本 target 高於 spot，方向為看漲。
  await expect(hero).toContainText("看漲");
  await expect(hero).toContainText("啟用的策略類型：Vertical Spread");
...
test("到期日結構：切換到期日 → 就地展開候選（V6／#54）", async ({ page }) => {
  ...
  // SW-10（#340）起劇本主圖排在候選列表之前（見同檔案「手機詳細頁
  // 整頁順序」測試），主圖內部的 `PriceLadder` 也是 `<li>` 結構——
  // 全頁 `getByRole("listitem").first()` 現在會先撈到那裡，改用
  // `.candidate-list` scope 回真正要驗的候選列表本身。
  const row = page.locator(".candidate-list").getByRole("listitem").first();
  // 三個價格在收合狀態就看得到
...
test("開站自動刷新：進度 → 卡片換成新數字（V4／#52，T08／#196 更新文案）", async ({ page }) => {
  ...
  // SW-10（#340，Owner 真機驗收）：「收益率以最差成交價計算」這句
  // 口徑說明已從主流程移除、整段搬進設定→免責聲明——首頁不再印這句話
  // 是刻意的裁示，不是回歸。
  await expect(page.getByText(/最差成交價/)).toHaveCount(0);
});
...
test("手機詳細頁不受桌面密度壓縮影響（QA-FIX-3／QA-01 的 Mobile 護欄）",
  ...
  // SW-10（#340，Owner 真機驗收）起「劇本設定」／「劇本摘要」兩張
  // `.summary-card` 密度卡已整段刪除——那份 12px 特別內距是那兩張卡
  // 專屬的規則，跟著一起移除，`MobileHero`（頭條 Hero 卡）用的是一般
  // `.card` 16px，沒有另外壓縮。這裡改成驗證頭條卡自己既有的
  // `.mobile-hero-stats` 兩欄格線在手機上真的換行（跟舊版「手機只有
  // 兩欄，不是桌面的四欄」同一個關注點，只是換了驗證對象——
  // `.mobile-hero-stats` 從來就不是響應式格線，手機／桌面都固定兩欄，
  // 因為它本來就只在手機分支渲染）。
  const hero = page.getByRole("region", { name: "劇本頭條" });
  const stats = hero.locator(".mobile-hero-stats .stat");
  const a = (await stats.nth(0).boundingBox())!;
  const b = (await stats.nth(1).boundingBox())!;
  expect(Math.abs(a.y - b.y)).toBeLessThan(2);
  expect(b.x).toBeGreaterThan(a.x);
  // 第三格要換行到下一列去，不是硬塞進同一列。
...
async function routeSettingsMobile(page: import("@playwright/test").Page) {
  // AUTH-06（#313）：credential 讀寫路徑掛在 Super Admin 閘門後——這裡
  // 既有測試檔測的是設定頁本身的資料流，不是登入流程本身，所以直接
  // 模擬「已登入 Super Admin」讓既有斷言照舊成立；登入流程自己的
  // 行為由 `Settings.test.tsx`（Vitest 元件層）專屬覆蓋。持久登入靠
  // `HttpOnly` cookie，前端不再需要（也不能）自己往 sessionStorage
  // 塞任何東西。
...
/** 詳細頁（含 Historical IV）與設定頁（含登入表單／跨 owner 管理面板）
 *  共用同一份角色狀態——`role` 活在這個函式的 closure 裡，
 *  `/api/auth/login`／`/api/auth/logout`／`/api/auth/status` 三個端點
 *  共讀共寫同一份，才測得出「在設定頁登入一次，詳細頁也立刻看得到
 *  效果」這種跨頁面的持久性，而不是三個各自獨立、永遠對不上的假回應。
 *  `historical_iv_enabled` 固定 `true`——這裡只在乎角色這一道閘門，
 *  「是否啟用」那道既有閘門（#126）已經有專屬測試覆蓋，不需要在這裡
 *  重複驗證。 */
async function routeRoleJourney(page: import("@playwright/test").Page) {
  let role: "normal" | "superuser" | "superadmin" = "normal";

  await routeLibrary(page, libraryRow(), sampleLongCall);
  await page.route("**/api/settings", (route) =>
    route.fulfill({ json: { ...SETTINGS_VIEW, historical_iv_enabled: true } }));
...
test("手機版：持久登入——登入後重新整理仍維持身分，不必重打密碼；登出後" +
...
test("手機版：角色可見度矩陣——Normal／Super User／Super Admin 三態下" +
...
/** 一份「一切正常」的完整 Historical IV 回應。2026-09-09 Owner 裁示
 *  （Vertical「貴不貴」整塊退場）後這塊卡片只服務單腿候選，因此預設
 *  只有買腿；`normalized_skew_points`／`metrics` 是後端契約仍然回傳的
 *  欄位（後端本輪未動），保留在這裡正好可以驗「就算回應帶了它，畫面
 *  也不該冒出 Normalized Skew」。個別測試需要 diagnostics events／
...
async function routeDetailWithIv(page: import("@playwright/test").Page,
                                 enabled: boolean,
                                 role: "normal" | "superuser" | "superadmin" = "superuser") {
  await routeLibrary(page, libraryRow(), sampleLongCall);
  // AUTH-06（#313）：`IvHistory.tsx` 現在疊了第三道角色閘門（#298／
  // #311），在既有的 `enabled`／`legs.length` 兩道之上——這裡預設
  // `superuser`，讓既有那批只測 #126 兩道閘門的測試維持原本行為，不
  // 需要逐一改動；本輪新增的角色可見度測試才會顯式傳 `"normal"`。
  await page.route("**/api/auth/status",
    (route) => route.fulfill({ json: { role } }));
...
async function routePartialHistory(
  ...
  await page.route("**/api/scenarios/*/iv-history*", (route) =>
    route.fulfill({ json: fullIvResponse({ legs: { buy: buyLeg } }) }));
}
...
test("Historical IV 解鎖：單腿候選一張逐腿卡為主要內容；Normalized Skew 已隨" +
     "2026-09-09 Owner 裁示整塊退場，連 Advanced 裡都不再出現（#114／HIVT-05／" +
     "SIG-02／#173）",
   async ({ page }) => {
  ...
  // 單腿候選只有一張逐腿卡（無「買腿」／「賣腿」標籤），是主要內容，
  // 不必展開任何東西就看得到。
  await expect(block.locator(".iv-trend-card")).toHaveCount(1);
  await expect(block.getByText(/第 41 百分位/)).toBeVisible();

  // Normalized Skew／Spread IV Gap 整組已退場——展開 Advanced 之後同樣
  // 一個字都不該出現（裁示明文不接受「用 Advanced 降級留下半套指標」）。
  await openAdvanced(block);
  await expect(block).not.toContainText(/Normalized Skew|Spread IV Gap|偏斜/);

  // 現值一律用 `.iv-value-primary`——單腿候選只剩這一個。
  ...
  expect(primarySizes).toHaveLength(1);
...
test("Historical IV：一年走勢圖在手機 viewport 完整可讀、寬度貼齊卡片（#140／#141）",
  ...
  // 2026-09-09 Owner 裁示：Vertical Spread 的「IV 相對位置」整塊退場，這塊卡片只服務單腿候選，
  // 因此整塊只剩逐腿卡片自己那一張走勢圖（原本是 Normalized Skew ＋
  // 買／賣腿共三張）。
...
test("Historical IV：Δ4w 帶正負號與單位，跟 percentile／涵蓋時間同一套事實敘述（#140／HIVT-05）",
  ...
  const block = page.locator(".iv-history");
  // 逐腿卡片：delta_4w = -0.012 → 「-1.2 pts」（主要區塊，不必展開 Advanced）
  await expect(block.getByText(/4週 -1\.2 pts/)).toBeVisible();
  // Normalized Skew 的無因次 Δ4w（「+0.01」）已隨它整組退場，展開
  // Advanced 之後也不該出現。
  await openAdvanced(block);
  await expect(block.getByText(/4週 \+0\.01/)).toHaveCount(0);
});
...
test("Historical IV 今日額度用完：percentile／Δ4w 照樣顯示，只多一行附加說明（#133／#140）",
  ...
  // 已快取的觀測算出的 percentile／Δ4w 照常顯示。2026-09-09 Owner 裁示
  // 之後 Normalized Skew 家族整組退場，這條斷言因此改掛在逐腿
  // exact-contract 家族自己的 `status` 上——同一個產品保證，換成它現在
  // 唯一還在畫面上的載體。
  await routeLibrary(page, libraryRow(), sampleLongCall);
  ...
  await page.route("**/api/scenarios/*/iv-history*", (route) =>
    route.fulfill({ json: fullIvResponse({
      legs: { buy: legHistoricalIv({
        status: "quota", note: "Market Data App 今日額度已用完" }) },
    }) }));
...
test("Historical IV 兩段式補建（T11／#194，P3-a）：backfill_pending 觸發" +
  ...
  // 後前端會重抓一次，這次回應換成補全後的新資料（逐腿百分位 41→66，
  // 驗證畫面真的換新，不是原地不動——原本用的 `observations` 屬於已經
  // 退場的 Normalized Skew 家族，畫面上讀不到了）。
  let ivCallCount = 0;
  await page.route("**/api/scenarios/*/iv-history*", (route) => {
    ivCallCount += 1;
    return route.fulfill({ json: fullIvResponse({
      ...
      legs: { buy: legHistoricalIv(
        { current_percentile: ivCallCount === 1 ? 0.41 : 0.66 }) },
    }) });
  });
  ...
  const block = page.locator(".iv-history");
  await expect(block).toBeVisible();
  // 補建進行中：卡片上有一句話說明，且舊資料仍然完整可看——不是整塊
  // 清空等新資料。
  await expect(block.getByText("歷史資料補建中……")).toBeVisible();
  await expect(block.getByText(/第 41 百分位/)).toBeVisible();

  // 補建結束後自動重抓：說明文字消失，畫面換成新一輪回應的數字。
  await expect(block.getByText("歷史資料補建中……")).toHaveCount(0);
  await expect(block.getByText(/第 66 百分位/)).toBeVisible();
...
test("Historical IV 完全沒有可比較觀測：誠實顯示沒有歷史資料，不硬湊（#133／#140／HIVT-05）",
   async ({ page }) => {
  // 2026-09-09 Owner 裁示後畫面上只剩逐腿 exact-contract 家族，「完全
  // 沒有觀測」因此就是這隻腳沒有觀測。
  await routeLibrary(page, libraryRow(), sampleLongCall);
  // AUTH-06（#313）：`IvHistory.tsx` 疊了角色閘門，這裡模擬已登入
  // Super User——這批既有測試驗的是 IV 卡片本身的呈現，不是登入
  ...
  // 沒有觀測就沒有東西可畫——走勢圖不畫，不是空白外框。
  await expect(block.locator(".iv-trend-chart")).toHaveCount(0);
  // 逐腿卡片是 HIVT-03（#154）明文要求的逐項 graceful
...
test("Historical IV：掛牌僅約 3 週的合約，涵蓋時間如實顯示「近 3 週」，不是" +
  ...
  const block = page.locator(".iv-history");
  const buyCard = block.locator(".iv-trend-card");
...
test("Historical IV：z-score／moving average／Bollinger 帶三項統計量在頁面上" +
  ...
  const block = page.locator(".iv-history");
  await expect(block).toBeVisible();
  // z-score caption：逐腿卡片預設 current_zscore 是 0.3 → "+0.30"——
...
test("手機版：Historical IV 卡片一開始就在，loading 時原位顯示骨架，資料回來後原位換成走勢圖" +
  ...
  releaseIv();
  // 骨架換成真正的主要內容（逐腿卡片，不必展開 Advanced 就看得到）。
  await expect(block.locator(".iv-trend-card")).toHaveCount(1);
...
test("手機版：多 family 並存——分頁列出、預設打開冠軍所屬 family、切換分頁"
  ...
  // 頭條（Hero 卡）固定顯示冠軍所屬 family——Vertical Spread（SW-10
  // 起 `Summary` 摘要卡已刪除，頭條顯示 family 層級，不是確切策略
  // 子類；確切子類「Bull Call Spread」仍在下面到期日排名表）。
  const hero = page.getByRole("region", { name: "劇本頭條" });
  await expect(hero.getByText(/劇本報酬 · Vertical Spread/)).toBeVisible();
...
test("SW-10（#340，Owner 真機驗收）：手機詳細頁整頁順序——劇本主圖" +
     "（Heatmap）在 Family tabs／排名表之前，三價位階梯／進場面板緊接" +
     "在排名表之後，Historical IV 在進場面板之後；進場面板固定顯示冠軍，" +
     "切換 family 分頁零額外請求、頭條與進場面板都不變。OG-10（#327）" +
     "當時把 Family tabs 排在劇本主圖之前——查證後那次排序依據的是已" +
     "退休的 Obsidian Gold artifact，不是 Seed Warm Direction A——" +
     "Owner 真機驗收明文指出「第二區原本應該有最佳劇本熱力圖，目前" +
     "不見，這是內容缺失」，這裡把 Heatmap 排回第二眼就看得到的位置", async ({ page }) => {
  const row = { ...libraryRow(), strategies: ["single-leg", "vertical-spread"] };
  const multi = multiFamilyView();
  await routeLibrary(page, row);
  await page.route("**/api/scenarios/s1", (route) =>
...
async function routeButterflyDetail(page: import("@playwright/test").Page) {
  ...
  // `supportsIvHistory`（腿數）本來就獨立於角色閘門擋下這個案例，
  // 這裡補上純粹是避免測試對一個未攔截的路由發出真實網路請求。
  const ivCalls: string[] = [];
  await page.route("**/api/auth/status",
    (route) => route.fulfill({ json: { role: "superuser" } }));
  await page.route("**/api/settings", (route) =>
...
test("T18（#235）紅線 12：展開一般 Vertical Spread 候選（非 Butterfly）" +
     "看得到 Heatmap，且不觸發任何額外網路請求——T16／#232 已驗過 Butterfly，" +
     "spec #217 這條紅線字面寫的是「任一前十名候選」，本條補上非 Butterfly " +
     "的代表案例，證明結構性保證（Heatmap.tsx 全檔零 fetch／useEffect，" +
     "展開走原生 <details>）不是只在 Butterfly 這一個 family 上成立",
   async ({ page }) => {
  const requestUrls: string[] = [];
  page.on("request", (req) => requestUrls.push(req.url()));
  await routeLibrary(page, libraryRow());
  ...
  // 教訓，T16／#232 的 Butterfly 版本測試已示範過一次，見上方）。
  await page.waitForLoadState("networkidle");
...
/* ---------- SW-10（#340）／PB-12（#302，Anonymous Public Beta）：首頁
   常駐頁尾＋隱私頁；Beta／cookie 說明已從首頁移入設定→免責聲明 ---------- */

test("手機版：首頁不再常駐 Beta 說明，頁尾在每個畫面都在，隱私頁與" +
     "設定→免責聲明皆可達",
   async ({ page }) => {
  await page.route("**/api/scenarios", (route) =>
    route.fulfill({ json: [] }));
  ...
  // SW-10（#340）：原本首頁常駐的四項事實搬進這裡——手機版設定頁是
  // 單欄堆疊，往下捲就看得到，不需要額外點分頁切換。
  await expect(page.getByRole("heading", { name: "免責聲明" })).toBeVisible();
