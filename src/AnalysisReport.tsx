 ...
 * - 原②「目標月/目標價/距現價」併入一句話結論，不獨立成段——
 *   `Summary` 已經顯示同一組數字（追平價格 S* 已依 spec #102 決策 E
 *   從畫面上整個移除，不再是「重複」的問題，見 #103）。
 ...
 * 同框，不可拆散。
 *
 * R1 §4.1 原骨架把「目標月/目標價/距現價」也放進②，這裡刻意不重複——
 * 本頁摘要區（`Summary`）已經在頁面上方無條件顯示同一組數字，分析報告
 * 是使用者主動展開才看得到的進階區塊，再說一次同一件事只是噪音，不是
 * 「結論先行」。追平價格 S* 已依 spec #102 決策 E 從畫面上整個移除
 * （#103）。
 */
