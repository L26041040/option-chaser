...
const KEY = "long-call|118|2026-09-18";
/** 兩腿候選的身份鍵——只服務 `verticalCandidate()`（退場驗證用）。 */
const VERTICAL_KEY = "bull-call-spread|118C|125C|2026-09-18";
...
/** 兩腿候選（Vertical Spread）——2026-09-09 Owner 裁示後，這塊功能對它
 *  整塊退場，因此這個 fixture 只剩「證明它真的不出現、也不發請求」這
 *  一個用途。 */
function verticalCandidate(): Candidate {
  return { candidate_key: VERTICAL_KEY, legs: [leg(), leg({ strike: 125 })] } as
    unknown as Candidate;
}

function longCallCandidate(): Candidate {
  return { candidate_key: KEY, legs: [leg()] } as unknown as Candidate;
}

/** 另一個單腿候選——`dataKey` 隔離測試需要兩個**不同身份鍵**的候選才
 *  測得出「切換候選時不誤用上一個候選的資料」。 */
function longPutCandidate(): Candidate {
  return { candidate_key: "long-put|110|2026-09-18",
          legs: [leg({ strike: 110, option_type: "put" })] } as
    unknown as Candidate;
}
 ...
 *  2026-09-09 Owner 裁示（Vertical 整塊退場）後，`ivView()` 自己已經是
 *  單腿形狀，這裡只多換一個對得上 `longCallCandidate()` 的身份鍵。 */
function singleLegIvView(over: Partial<IvHistoryView> = {}): IvHistoryView {
  return ivView({ metrics: emptyMetrics(), ...over });
}

/** SIG-01（#172）`spread_gap` 區塊——後端契約本輪未動，兩腿候選的回應
 *  仍然帶著它；2026-09-09 Owner 裁示後前端不再有任何消費端，這份
 *  fixture 只剩下「就算回應帶了它，畫面上也不該冒出 Spread IV Gap」
 *  這個負向斷言用得到。 */
...
/** 「卡片內容已經到位」的哨兵——逐腿卡片的百分位（`legHistoricalIv()`
 *  預設 `current_percentile: 0.5`）。2026-09-09 Owner 裁示前這裡用的是
 *  Normalized Skew 頭條，那塊已整塊退場，改用這塊卡片現在真正的主體。 */
const LEG_LOADED = /第 50 百分位/;
...
describe("第二個閘門：只服務單腿候選（Butterfly＝T16／#232 結構上不支援；" +
  ...
  it("兩腿候選（Vertical Spread）也不輸出任何 DOM 節點——Owner 裁示整塊" +
     "退場，不是搬進 Advanced 降級顯示", async () => {
    const urls = mockApi({ enabled: true });
    const { container } = render(
      <IvHistory scenarioId="s1" candidate={verticalCandidate()} />);
    await waitFor(() => expect(urls.some((u) => u.startsWith("/api/settings")))
      .toBe(true));
    expect(container).toBeEmptyDOMElement();
  });

  it("兩腿候選一個 IV 請求都不發——退場是在請求層擋下，不是抓回來再藏起來",
     async () => {
    const urls = mockApi({ enabled: true });
    render(<IvHistory scenarioId="s1" candidate={verticalCandidate()} />);
    await waitFor(() => expect(urls.some((u) => u.startsWith("/api/settings")))
      .toBe(true));
    expect(ivCalls(urls)).toEqual([]);
  });

  it("兩腿候選畫面上不出現 Spread IV Gap／Normalized Skew 任何殘留" +
     "（含 Advanced 收合區裡）", async () => {
    const urls = mockApi({ enabled: true,
                          iv: ivView({ spread_gap: spreadGapFixture() }) });
    const { container } = render(
      <IvHistory scenarioId="s1" candidate={verticalCandidate()} />);
    await waitFor(() => expect(urls.some((u) => u.startsWith("/api/settings")))
      .toBe(true));
    expect(container.textContent ?? "").not.toMatch(
      /Spread IV Gap|Normalized Skew|偏斜|IV 相對位置/);
  });

  it("換成單腿候選就恢復正常請求——閘門只針對非單腿，不是永久壞掉", async () => {
    const urls = mockApi({ enabled: true });
...
describe("已有可用 cache 時，重新嘗試失敗只降級成非阻斷警示（需求方 2026-08-21 反饋：" +
  ...
  it("同一個候選第一次成功、新分析後 refetch 失敗：圖表照常在，只多一行警示，" +
    ...
    const { container, rerender } = render(
      <IvHistory scenarioId="s1" candidate={longCallCandidate()} analyzedAt="t1" />);
    await waitFor(() =>
      expect(screen.getByText(LEG_LOADED)).toBeInTheDocument());

    rerender(
      <IvHistory scenarioId="s1" candidate={longCallCandidate()} analyzedAt="t2" />);
    await waitFor(() =>
...
describe("切換候選時不會誤用上一個候選的資料（dataKey 隔離）", () => {
  it("候選 A 成功後切到候選 B，B 的 fetch 還沒回來前顯示骨架，不是 A 的舊資料",
    ...
    rerender(<IvHistory scenarioId="s1" candidate={longPutCandidate()} />);
    await waitFor(() =>
      expect(container.querySelector(".iv-skeleton")).toBeInTheDocument());
    expect(screen.queryByText(LEG_LOADED)).not.toBeInTheDocument();

    resolveSecond({ ok: true, status: 200,
                   json: async () => singleLegIvView(
                     { candidate_key: "long-put|110|2026-09-18" }) } as Response);
    await waitFor(() =>
...
describe("legacy normalized_skew 失敗不污染 exact-contract 逐腿卡片（C.3）", () => {
  it("頂層 status 為 vendor（legacy backfill 沒補上）時，逐腿卡片照常渲染" +
     "成功——2026-09-09 Owner 裁示後 legacy 家族連 Advanced 裡都不再顯示，" +
     "它的失敗因此對畫面完全沒有影響", async () => {
    ...
    // legacy 家族自己的說明（頂層 status／note／spread_gap）一個字都不
    // 該冒出來——包含展開 Advanced 之後。
    const summary = await screen.findByText("Advanced／Diagnostics");
    await userEvent.click(summary);
    expect(container.textContent ?? "")
      .not.toMatch(/資料源暫時無法連線|Spread IV Gap|Normalized Skew/);
  });
});
...
describe("只陳述事實（紅線，由測試守門而非自律）", () => {
  ...
  it("backfill 說明同樣不帶評價", async () => {
    mockApi({ enabled: true, iv: ivView({
      legs: { buy: legHistoricalIv({ status: "quota" }) } }) });
    const { container } = render(
      <IvHistory scenarioId="s1" candidate={longCallCandidate()} />);
    await waitFor(() =>
...
describe("固定版位，不因 request 完成才決定要不要出現（QA 反饋，2026-08-16）", () => {
  ...
  it("骨架固定是單腳形狀：一個次層方塊、沒有 Normalized Skew 頭條方塊" +
     "（2026-09-09 Owner 裁示後這塊卡片只服務單腿候選）", async () => {
    ...
    expect(singleContainer.querySelectorAll(".iv-skeleton-secondary"))
      .toHaveLength(1);
    expect(singleContainer.querySelectorAll(".iv-skeleton-primary"))
      .toHaveLength(0);
  ...
  it("逐腿卡片無資料（percentile 為 null）時卡片照常在，顯示「沒有歷史資料」" +
     "而不是整塊消失", async () => {
    mockApi({ enabled: true, iv: ivView({
      legs: { buy: legHistoricalIv({ current_percentile: null }) } }) });
    const { container } = render(
      <IvHistory scenarioId="s1" candidate={longCallCandidate()} />);
    await waitFor(() =>
...
describe("Advanced／Diagnostics 收合區（SIG-02／#173）", () => {
  ...
  it("展開後看得到逐腿 z-score 文字；Normalized Skew 整組已隨 2026-09-09 " +
     "Owner 裁示退場，不在這裡降級顯示", async () => {
    mockApi({ enabled: true });
    const { container } = render(
      <IvHistory scenarioId="s1" candidate={longCallCandidate()} />);
    const summary = await screen.findByText("Advanced／Diagnostics");
    ...
    expect(container.textContent ?? "")
      .not.toMatch(/Normalized Skew|偏斜|Spread IV Gap/);
  });
...
describe("兩段式補建（T11／#194，P3-a）——Legacy 家族冷 backfill 不再同步夾在 GET 裡", () => {
  ...
  it("補建進行中顯示「歷史資料補建中」，完成後自動重抓並消失", async () => {
    const { releaseBackfill } = mockApiTwoPhase(
      (i) => ivView({
        backfill_pending: i === 0,
        legs: { buy: legHistoricalIv(
          { current_percentile: i === 0 ? 0.5 : 0.66 }) },
      }));
    render(<IvHistory scenarioId="s1" candidate={longCallCandidate()} />);
    ...
    await waitFor(() =>
      expect(screen.queryByText("歷史資料補建中……")).not.toBeInTheDocument());
    // 補建完成後自動重抓：畫面換成新一輪回應（百分位從 50 變 66）的內容。
    await waitFor(() =>
      expect(screen.getByText(/第 66 百分位/)).toBeInTheDocument());
  });
