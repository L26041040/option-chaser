 ...
 * 隨便挑的數字。這個封頂值本身沒有變動。
 *
 * SW-10（#340，Owner 真機驗收）：`t` 原本直接線性對到 alpha
 * （`t * 0.72`）——一張期權報酬率 Heatmap 大多數格子落在 |報酬率|
 * 5%–40% 這個中低區間（真正逼近或超過 ±100% 封頂的格子通常只有表格
 * 一角），線性曲線在這段區間給的 alpha 只有 0.036–0.29，長時間盯著
 * 一大片幾乎看不出顏色的格子找漲跌方向確實吃力——這正是 Owner 點名
 * 「現在色階太淡、長時間看很累」的量化原因，不是主觀感受。這裡把 `t`
 * 先過一個 `t^0.7` 的次方曲線再乘封頂值：低－中報酬率的格子因此提早
 * 爬到看得出顏色的濃度（例如 10% 報酬率原本 alpha≈0.07，現在
 * ≈0.15），封頂值本身（`t=1` 時 `1^0.7=1`）完全不變，`contrast.
 * test.ts` 鎖住的「最深格文字仍達 WCAG AA」不受影響。指數選 0.7 而非
 * 更激進的 0.5：0.5 在 |報酬率| 剛跨過 `NEUTRAL_BAND`（5%）時會讓
 * alpha 瞬間跳到 0.16 左右，色階邊界處會有一道不自然的「斷層」，跟
 * Owner 同時要求的「不要重新搞成刺眼 terminal 風」牴觸；0.7 在邊界
 * 附近仍平滑爬升，只是比純線性更快到達看得清楚的濃度。 */
export function cellColor(ret: number): string {
  ...
  const alpha = Math.pow(t, 0.7) * 0.72;
...
// QA 修正：`<超標>`／`<深跌>` 已從引擎移除（憑空生出來的價位，對使用者
// 沒有意義），換成使用者自己在建立劇本時填的最高／最低價位。
const TAGS: Record<string, string> = {
  "<現價>": "現價", "<目標>": "目標", "<最高>": "最高", "<最低>": "最低",
};
