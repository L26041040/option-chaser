...
級別的系統偏差）。**最可能的解釋**（【本文推導】）：3-DTE 下 vega
塌縮造成的數值不穩定本身就會產生系統性偏差，不需要額外假設 q／r
算錯——但這只是推論，不是本次已經隔離變數驗證過的結論；有 medium／
LEAPS 樣本可以直接檢驗這個假說是否成立（若偏差在較長天期大幅縮小，
支持這個解釋；若偏差量級不變，代表另有系統性原因，需要重新檢視
recipe 本身）。
