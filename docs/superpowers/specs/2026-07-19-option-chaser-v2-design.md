...
**單腿**（long vega 恆正，v1 語意成立）：

| 策略 | Breakeven | 保守底線（劇本日）= L1 硬上限 |
|---|---|---|
| long-call | K + Mid | max(target − K, 0) |
| long-put | K − Mid | max(K − target, 0) |

- 單腿 L2 = 最保守 IV 情境（`min(iv_shifts)`）的劇本日估值（含 §3.2 鉗制）；L3 = 基準估值 ÷ (1 + min_return)。恆等式 `L1 ≤ L2 ≤ 基準` 在鉗制後成立，測試以固定參數矩陣驗證。

**價差**（重要差異——單腿直覺不可平移）：垂直價差淨 vega 會變號（深價內時 IV 下降反使價值上升趨近寬度），且到期前短腿仍有時間價值，target 日 payoff 不構成估值下限。因此：

| 策略 | Breakeven | 買價指引 |
|---|---|---|
| bull-call-spread | K_low + 淨Mid | **無 L1**（到期前不存在 model-free 下限）；L2 = 全部 IV 情境估值的最小值（含基準情境，故恆等式 `L2 ≤ 基準` 結構性成立）；L3 = 基準估值 ÷ (1 + min_return) |
| bear-put-spread | K_high − 淨Mid | 同上 |

- 價差報告的「保守底線」行改標示為「IV 情境最低值」（= L2），尾註註明其語意與單腿不同（情境包絡，非無套利下限）。
- 對目標價緩衝：call 族 = `(target − BE)/target`；put 族 = `(BE − target)/target`（正值 = 劇本兌現仍獲利）。
- 價差結構性上界：淨成本恆 < 寬度（無套利）；`Ask 口徑成本 ≥ 寬度` 的組合已於配對階段淘汰，不會出現在報告。
...
1. **腿粗掃**：候選腿先過 §4.1 五道過濾。此為刻意的流動性/資料品質約束：窮舉的完備性是「合格腿空間」內的完備（零漏解），不宣稱涵蓋含不合格腿（壞報價/低流動性）的組合——那些組合即使帳面報酬更高也不可交易，排除是產品決策而非漏算。
...
- **價格軸（11 列，由高至低顯示）**：範圍 `lo = max(min(spot, target) − pad, 0.01 × spot)`、`hi = max(spot, target) + pad`，`pad = 0.10 × spot`（下限鉗制防止低目標價 put 情境產生 ≤ 0 股價，避免 `ln(S/K)` 定義域錯誤）。
- **錨點植入演算法（防碰撞，確定性）**：anchors = 去重後的 {spot, target}（相等時只有一個）。先生成 11 個等距格點；依 anchors 升冪逐一處理：每個 anchor 移除「尚未被移除的格點中距其最近者」（距離同分取較低格點），再插入 anchor 本身。結果恰 11 列、每個 anchor 必然以精確值存在且標記（`<現價>`/`<目標>`；spot == target 時同列雙標記）。
- **日期軸（7 欄）**：`今天 → expiry` 日曆日等分（含兩端）。anchor = `target_date`：若等於任一端點（`expiry == target_date` 為合法情況），該端點欄直接加 `*` 標記不另生欄；否則以同上防碰撞演算法對內部格點移除+插入。末欄恆為 expiry。
- 兩軸皆為 snapshot + params 的純函數；同輸入逐位元同輸出。碰撞案例（spot 與 target 相近、target_date == expiry）必須有專屬單元測試。
...
7. **買價指引恆等式**：單腿兩策略 × 固定參數矩陣驗證 `L1 ≤ L2 ≤ 基準`（鉗制後）；價差兩策略驗證 `L2 = min(全部 IV 情境) ≤ 基準`（結構性）與「無 L1」；另需一個「深價內價差在 IV 下降時價值上升」的反直覺案例測試（鎖住淨 vega 變號事實，防止未來有人把單腿 L2 定義誤搬回價差）。
...
<!-- codex-peer-reviewed: 2026-07-18T20:12:34Z rounds=2 verdict=approved -->
