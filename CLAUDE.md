...
**第三輪反饋已到、尚未分析拆票**：`docs/user-feedback-v3.md`
（2026-08-02，需求方測試部署版後由 GitHub 直接 commit 進 master）。
共 10 點，尚未逐點確認需求，其中特別注意：

- **第 4 點是嚴重 bug 回報**：非 baseline 到期日的收益率疑似算錯
  （「只有劇本設定日附近那個到期日會準，其他都似乎只算成劇本維持
  原價的最高收益」，TLT 2028/5 120 實例與 OPC 對不上）——這是計算
  引擎正確性問題，優先級高於一切前端工作，應最先調查
- 多點與下一階段前端重練重疊（第 5 點 iOS 風格＝已裁示方向、
  第 7 點釘選功能列＝QA1-14 範圍），拆票時要與新前端 spec 合併考量，
  避免在舊 Streamlit 前端上做白工
