...
**修法方向（未裁示，勿自行施工）**：
1. `docs/research/option-chain-data-sources.md` §4.3 早就建議、當時被
   判斷「先不做」的 **last-price 降級口徑**——bid 缺失時改用最後成交價，
   但**必須顯式標記為降級資料**（last 可能是好幾天前的成交，無聲拿它
   當成本會算出假的收益率）
2. 新前端目前**沒有**「候選池過少」警示（Streamlit 版的 FB3-02 有，
   新版屬 V6／#54 範圍）——在修好之前，盤後結果是無聲誤導
