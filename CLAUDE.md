...
- **SW-07 #336** — Forms/Settings/Privacy/Login/Super Admin: `CreateForm.tsx` submit/cancel → `.pbtn`/`.pbtn.line`, family checkboxes visually chip-ified (`.family-option.selected`, real `<input type=checkbox>` kept for a11y). `Settings.tsx`'s save/test buttons → `.pbtn`; `RoleLogin.tsx`'s submit → `.pbtn`. `DeleteMyData.tsx` gets a new `.danger-zone` red-outline card + `.batch-pill.danger` confirm button (its confirm modal has no type-to-confirm text input and never did — only `SuperUserAdmin.tsx`'s `ConfirmHighRiskAction` has that mechanism, left untouched). `SuperUserAdmin.tsx`'s `OpsStats` converted from bare `.stat` rows to `.pstat` cards (new `.pstat-grid`, auto-fill) to match the sibling `UsageStatsStrip` on the library page — caught by `/code-review` Spec axis as a real gap, not deliberate. `.input`/`.settings-input`/`.pinp` and the month-picker were investigated and found to already carry correct Seed Warm colors via SW-01's token aliasing; `.pinp` itself is deliberately NOT applied to full-width form fields (would regress the 16px+ iOS-zoom-prevention sizing) — disclosed via code comments, not a silent gap. `.month-cell` radius bumped 8px→12px for shape-language consistency. `PrivacyPage.tsx`/`SuperUserAdmin.tsx`'s ops-metrics colors and permission matrix otherwise untouched (already compliant / out of touched scope respectively). "Vendor 每日預算" stays Super-Admin-only per the spec's own text. New jargon-scan tests (CreateForm/PrivacyPage/Settings-as-Normal-User) + new 375px/1440px no-horizontal-scroll e2e for Settings/Privacy (previously uncovered). Zero backend/API changes.
- **SW-05 #337** — Desktop detail page: identity row gets a new champion-return row (800-weight large return + "劇本報酬 · family" subtitle, computed inline via `championCandidate(detail.latest_result)`) above the existing 5-cell meta row, which itself collapsed to the same 4-stat shape as mobile's `MobileHero` (現價/還需/距目標/來源+時間; target price+month moved into the new return row). `.chip`'s pill-radius CSS (previously scoped `@media (max-width: 1099px)` only, since SW-05 hadn't run when SW-06 added it) is now unconditional — `DesktopDetail.tsx`'s own family/expiry/right-panel/bottom-tab chips are all pill-shaped now too; `.settings-subnav .chip` explicitly pins `padding: 8px 12px` since it doesn't re-declare that property and would otherwise silently inherit the base rule's new value (`/code-review` Standards axis catch). `DesktopDetail.tsx`'s own independent family-tab JSX (doesn't reuse `FamilyTabs.tsx`'s render, only its exported pure functions) gained the same `aria-hidden` champion badge mobile got in SW-06 (`/code-review` Spec axis catch — was missed on the first pass). `.detail-rank-row.selected` → `--accent-soft` (was generic `--fill`). Wording: `DesktopDetail.tsx`'s bottom-tab label "候選池診斷"→"候選策略" (now literally the same string as `CandidatePool.tsx`'s own already-renamed heading — tests disambiguate via Testing-Library's `{selector: "h2"}` / Playwright's body-scoped locator) and EntryTab's "最差成交口徑"→"以最差成交價計算". Kept `aria-pressed` (not `aria-selected`) on ranking rows — `aria-selected` isn't valid ARIA on a plain `<button>`, disclosed via comment. No company name shown (same disclosed no-data-source gap as SW-06's mobile hero). PriceLadder intentionally stays inside the same `.card` as the Heatmap (not split into its own card) — mirrors `ScenarioDetail.tsx`'s own documented mobile/desktop parity for this exact pairing, not an oversight. New 1440px 3-column no-horizontal-scroll + tab-switch-visibility e2e (previously uncovered). Zero backend/API changes.

- **SW-08 #338** — Charts/data-viz recolor: `heatmap.ts::cellColor()`'s hardcoded `GAIN`/`LOSS` RGB triplets swapped from old iOS-green/red literals to the exact Seed Warm `--up`/`--down` hex values; max alpha cut 0.8→0.72 after a new `contrast.test.ts` check (worst-case blended cell vs `--text`) failed by 0.0025 at 0.8 for the down/red case. `Heatmap.tsx` rows now get `anchor-target`/`anchor-spot` classes (can both apply to one row) driving new `--acc-text`/`--soft` row highlighting — previously both shared one undifferentiated `.anchor` class (caught by `/code-review` Spec axis as a real gap). `IvTrend.tsx`'s desktop percentile `<p>` → new `.iv-percentile-caption` (`--acc-text`) — mobile's combined percentile+Δ4w sentence left alone (not structurally splittable without a bigger, riskier change). Added `font-variant-numeric: tabular-nums` to the 3 SVG-text chart CSS classes (`.chart-axis-unit`/`.chart-tick-label`/`.chart-tooltip text`) — the Heatmap `<table>` already had it and inherits, SVG `<text>` doesn't. `--suspect` recolored `#c0392b`→`#9c3b1f` (kept as its own token, not merged into `--warn` — different severity, spec says keep it distinguishable). `CostSparkline.tsx` line width 1.5→2 + round linecaps. Renamed old-era CSS var references (`--label*`/`--separator`/`--bg-elevated`/`--orange`/`--tint`/`--text-3`/bare `--accent`) to their canonical Seed Warm aliases across all chart-adjacent CSS rules — pure renames, same underlying hex values throughout (traced every alias chain). New `src/chartTokens.test.ts` statically scans all 14 chart/data-viz source files (.tsx + their pure-function .ts helpers) for old-token `var()` references and literal "gold" — all clean. Disclosed: no mobile e2e for CostSparkline exists because it structurally never renders on mobile (`CompactScenarioList.tsx` has no sparkline column, pre-existing OG-04/SW-03 decision) — not a coverage gap, there's nothing there to cover. `CandidatePool.tsx`/`RawData.tsx`/`Diagnostics.tsx` untouched — confirmed zero inline color code in their own source (only consume globally-shared, already-token-correct classes). Zero backend/API changes.
