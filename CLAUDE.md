...
- 利率**先接上再選源**：不綁死 `home.treasury.gov`（需求方指出 Yahoo 的
  API 連得上，代表不是部署環境在擋，可能是國債網站本身不給 API）。
  Treasury／FRED／Yahoo 比較可靠性與完整度後再定主備援；一輪刷新全部
  劇本共用同一條曲線；快取放 Neon（serverless 檔案系統唯讀，現有檔案
  快取那層是死的）
- 進階區舊 cache 以資料正確性優先，**刷新後收合重取可接受**

**票與依賴**（← 為可立刻開工）：

- **#67** 利率：production 接上利率曲線，並誠實區分三態 ←
- **#68** 過期劇本不再進入批次刷新 ←
- **#69** 進階區資料隨新分析失效，不得混用新舊 cache ←
- **#70** 詳細頁補上刷新入口 ←
- **#71** 自製年月選擇器 ←
- **#72** 桌面版真正的 master/detail ←
- **#73** Research：公開利率資料源評選（被 #67 擋——需要它掛上的健康
  檢查探針才能實證部署環境可達性）
- **#74** 利率：換上選定資料源並硬化抓取（被 #73 擋）
- **#75** 主要操作入口收攏到工作區上方（被 #72 擋）

> ⚠ **沙箱對所有利率端點都回 403**（與 `cdn.cboe.com`、`*.vercel.app`
> 同一個閘道限制，已多次確認，不要再嘗試繞）。#67 刻意把利率來源與
> 失敗原因掛到健康檢查端點——**部署版是唯一能驗證可達性的地方**，
> 那個端點就是 #73 的探針。
